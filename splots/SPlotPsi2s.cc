...
SPlotPsi2s::SPlotPsi2s(const char* name, const char* title):
  TNamed(name, title)
{
  RooArgList Args;
...
SPlotPsi2s::SPlotPsi2s(const char* name, const char* title, const RooDataSet &data):
  TNamed(name, title)
{
  RooArgList Args;
...
////////////////////////////////////////////////////////////////////////////////
/// Copy Constructor from another SPlotPsi2s

SPlotPsi2s::SPlotPsi2s(const SPlotPsi2s &other):
  TNamed(other)
{
...
SPlotPsi2s::SPlotPsi2s(const char* name, const char* title, RooDataSet& data, RooAbsPdf* pdf,
  ...
  TNamed(name, title)
{
  ...
  while((arg=(RooAbsArg*)iter->Next())) {
    if (!dynamic_cast<RooRealVar*>(arg)) {
      coutE(InputArguments) << "SPlotPsi2s::SPlotPsi2s(" << GetName() << ") input argument "
             << arg->GetName() << " is not of type RooRealVar " << endl ;
      throw string(Form("SPlotPsi2s::SPlotPsi2s(%s) input argument %s is not of type RooRealVar",GetName(),arg->GetName())) ;
    }
  }
  delete iter ;

  //Construct a new SPlotPsi2s class,
...
RooDataSet* SPlotPsi2s::SetSData(RooDataSet* data)
{
  if(data)    {
    fSData = (RooDataSet*) data;
    return fSData;
...
RooDataSet* SPlotPsi2s::GetSDataSet() const
{
  return fSData;
}
...
Double_t SPlotPsi2s::GetSWeight(Int_t numEvent, const char* sVariable) const
{
...
Double_t SPlotPsi2s::GetSumOfEventSWeight(Int_t numEvent) const
{
  if(numEvent > fSData->numEntries() )
    {
      coutE(InputArguments)  << "Invalid Entry Number" << endl;
...
Double_t SPlotPsi2s::GetYieldFromSWeight(const char* sVariable) const
{
...
RooArgList SPlotPsi2s::GetSWeightVars() const
{
...
Int_t SPlotPsi2s::GetNumSWeightVars() const
{
  RooArgList Args = fSWeightVars;
...
///
/// The SPlotPsi2s will contain two new variables for each specie of name "varname":
///
...
void SPlotPsi2s::AddSWeight( RooAbsPdf* pdf, const RooArgList &yieldsTmp,
         const RooArgSet &projDeps, bool includeWeights)
{
  ...
  for(Int_t i = 0; i < constParameters->getSize(); i++) {
    ...
    // cout << "SPlotPsi2s constParameters before: " << varTemp->GetName() << " " << varTemp->getVal() << endl;
  }
  ...
  for(Int_t i = 0; i < constParameters->getSize(); i++) {
    RooRealVar* varTemp = ( dynamic_cast<RooRealVar*>( constParameters->at(i) ) );
    // cout << "SPlotPsi2s constParameters after: " << varTemp->GetName() << " " << varTemp->getVal() << endl;
  }
  ...
  // This is necessary because SPlotPsi2s assumes the yields minimise -Log(likelihood)
  ...
  for (Int_t ievt = 0; ievt <numevents; ievt++) {
	...
	yieldvars[k]->setMax(1);
      ...
      // set this yield to 1
      // !!!  ===> chiara: for K=3 (i.e. K*Psi2s bkg) set also nsignal=1 <=== !!!
	...
	cout << "SPlotPsi2s varPippo: " << varPippo->GetName() << " " << varPippo->getVal() << endl;
	pdf->Print(); 
	cout << "SPlotPsi2s pdf->getVal = " << pdf->getVal(&vars) << endl;
      }
      */
      //**************
  ...
  if (covInv.Determinant() <=0)
    {
      coutE(Eval) << "SPlotPsi2s Error: covariance matrix is singular; I can't invert it!" << std::endl;
      covInv.Print();
      return;
    }
