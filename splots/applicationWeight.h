...
#ifndef applicationWeight_h
#define applicationWeight_h
...
class applicationWeight {
 public :
  ...
  applicationWeight(TTree *tree=0);
  virtual ~applicationWeight();
  virtual Int_t    Cut(Long64_t entry);
  virtual Int_t    GetEntry(Long64_t entry);
  virtual Long64_t LoadTree(Long64_t entry);
  virtual void     Init(TTree *tree);
  virtual void     Loop(const char* ratioFileName, int q2bin);
  virtual Bool_t   Notify();
  virtual void     Show(Long64_t entry = -1);
  float GetAnBdtWeight(float bdt); 

  // For weights application
  std::vector<Double_t> anweight_;  
  std::vector<Double_t> lowedge_;  
};

#endif

#ifdef applicationWeight_cxx
applicationWeight::applicationWeight(TTree *tree) : fChain(0) 
{
...
applicationWeight::~applicationWeight()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t applicationWeight::GetEntry(Long64_t entry)
{
...
Long64_t applicationWeight::LoadTree(Long64_t entry)
{
...
void applicationWeight::Init(TTree *tree)
{
...
Bool_t applicationWeight::Notify()
{
  return kTRUE;
}

void applicationWeight::Show(Long64_t entry)
{
...
Int_t applicationWeight::Cut(Long64_t entry)
{
...
#endif // #ifdef applicationWeight_cxx
