...
double GetKptWeight(double kpt);
...
std::vector<Double_t> anweightKpt_;
std::vector<Double_t> anweightElesDr_;
std::vector<Double_t> lowedgeEle1pt_;
std::vector<Double_t> lowedgeEle2pt_;
std::vector<Double_t> lowedgeKpt_;
std::vector<Double_t> lowedgeElesDr_;

void weightAdditionToTree(const char* dataFilename, const char* ratioFilename) {

  cout << "Formatting " << dataFilename << " adding weights" << endl;  
  ...
  for(int i=0; i<(int)trees.size();i++) {
    ...
    theTreeNew->Branch("weightElePt1", &weightElePt1, "weightElePt1/D");
    theTreeNew->Branch("weightElePt2", &weightElePt2, "weightElePt2/D");
    theTreeNew->Branch("weightKPt", &weightKPt, "weightKPt/D");
    theTreeNew->Branch("weightElesDr", &weightElesDr, "weightElesDr/D");
    theTreeNew->Branch("xgb", &xgb, "xgb/D");
    theTreeNew->Branch("Bmass", &Bmass, "Bmass/F");
    theTreeNew->Branch("Mll", &Mll, "Mll/F");
    theTreeNew->Branch("Npv", &Npv, "Npv/F");
    theTreeNew->Branch("Bprob", &Bprob, "Bprob/F");
    theTreeNew->Branch("BsLxy", &BsLxy, "BsLxy/F");
    theTreeNew->Branch("L1pt", &L1pt, "L1pt/F");
    theTreeNew->Branch("L2pt", &L2pt, "L2pt/F");
    theTreeNew->Branch("Kpt", &Kpt, "Kpt/F");
    theTreeNew->Branch("Bcos", &Bcos, "Bcos/F");
    theTreeNew->Branch("LKdz", &LKdz, "LKdz/F");
    theTreeNew->Branch("L1L2dr", &L1L2dr, "L1L2dr/F");
    theTreeNew->Branch("LKdr", &LKdr, "LKdr/F");
    theTreeNew->Branch("L2id", &L2id, "L2id/F");
    theTreeNew->Branch("Kiso", &Kiso, "Kiso/F");
    theTreeNew->Branch("BBDphi", &BBDphi, "BBDphi/F");
    theTreeNew->Branch("BTrkdxy2", &BTrkdxy2, "BTrkdxy2/F");
    theTreeNew->Branch("L1id", &L1id, "L1id/F");
    theTreeNew->Branch("L1iso", &L1iso, "L1iso/F");
    theTreeNew->Branch("KLmassD0", &KLmassD0, "KLmassD0/F");
    theTreeNew->Branch("Passymetry", &Passymetry, "Passymetry/F");
    theTreeNew->Branch("Kip3d", &Kip3d, "Kip3d/F");
    theTreeNew->Branch("Kip3dErr", &Kip3dErr, "Kip3dErr/F");
    theTreeNew->Branch("Bpt", &Bpt, "Bpt/F");
    theTreeNew->Branch("Mu7_IP4", &Mu7_IP4, "Mu7_IP4/F");
    theTreeNew->Branch("Mu8_IP3", &Mu8_IP3, "Mu8_IP3/F");
    theTreeNew->Branch("Mu8_IP5", &Mu8_IP5, "Mu8_IP5/F");
    theTreeNew->Branch("Mu8_IP6", &Mu8_IP6, "Mu8_IP6/F");
    theTreeNew->Branch("Mu9_IP5", &Mu9_IP5, "Mu9_IP5/F");
    theTreeNew->Branch("Mu9_IP6", &Mu9_IP6, "Mu9_IP6/F");
    theTreeNew->Branch("Mu12_IP6", &Mu12_IP6, "Mu12_IP6/F");
  }
  ...
  // Load weights: K pt
  for (int i = 0; i<ratioKpt_fromFile->GetNbinsX(); i++) {

    double myweightKpt=ratioKpt_fromFile->GetBinContent(i+1);
    anweightKpt_.push_back(myweightKpt);              

    double mylowedgeKpt=ratioKpt_fromFile->GetBinLowEdge(i+1); 
    lowedgeKpt_.push_back(mylowedgeKpt);
  }
  ...
  for(int i=0; i<nentriesOrig; i++) {
    ...
    double OneOverWeightKpt    = GetKptWeight(Kpt);
    double OneOverWeightElesDr = GetElesDrWeight(L1L2dr);
    double thisEventWeightEle1pt, thisEventWeightEle2pt, thisEventWeightKpt, thisEventWeightElesDr;
    ...
    if (OneOverWeightKpt!=0) 
      thisEventWeightKpt = 1./OneOverWeightKpt;
    else
      thisEventWeightKpt = 0;
    ...
    weightElePt1 = thisEventWeightEle1pt;
    weightElePt2 = thisEventWeightEle2pt;
    weightKPt    = thisEventWeightKpt;
    weightElesDr = thisEventWeightElesDr;
...
double GetKptWeight(double kpt) {
  ...
  int thesizem1 = lowedgeKpt_.size()-1;
  
  double weight=1;
  for (int i = 0; i<thesizem1; i++) {   
    if (lowedgeKpt_[i]<=kpt && lowedgeKpt_[i+1]>kpt) weight = anweightKpt_[i];  
  }
