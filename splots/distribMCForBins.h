...
#ifndef distribMCForBins_h
#define distribMCForBins_h
...
class distribMCForBins {
 public :
  ...
  distribMCForBins(TTree *tree=0);
  virtual ~distribMCForBins();
  virtual Int_t    Cut(Long64_t entry);
  virtual Int_t    GetEntry(Long64_t entry);
  virtual Long64_t LoadTree(Long64_t entry);
  virtual void     Init(TTree *tree);
  virtual void     Loop(int q2bin);
...
#ifdef distribMCForBins_cxx
distribMCForBins::distribMCForBins(TTree *tree) : fChain(0) 
{
  ...
  if (tree == 0) {          
    ...
    //TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ottoPFLPnonReg/forMeas_xgbmodel_kee_12B_kee_correct_pu_Depth17_LowPtPF_v7.3_nonreg_0_MC.root");            // Low-q2 PFLP
    //TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ottoPFLPnonReg/forMeas_xgbmodel_kee_12B_kee_correct_pu_Depth17_LowPtPF_v7.3_nonreg_0_MCPsi2S.root");       // Psi2s PFLP
    if (!f || !f->IsOpen()) {
      ...
      //f = new TFile("ottoPFLPnonReg/forMeas_xgbmodel_kee_12B_kee_correct_pu_Depth17_LowPtPF_v7.3_nonreg_0_MC.root");                 // Low-q2 PFLP  
      //f = new TFile("ottoPFLPnonReg/forMeas_xgbmodel_kee_12B_kee_correct_pu_Depth17_LowPtPF_v7.3_nonreg_0_MCPsi2S.root");            // Psi2s PFLP  
    }
...
distribMCForBins::~distribMCForBins()
{
  if (!fChain) return;
  delete fChain->GetCurrentFile();
}

Int_t distribMCForBins::GetEntry(Long64_t entry)
{
...
Long64_t distribMCForBins::LoadTree(Long64_t entry)
{
...
void distribMCForBins::Init(TTree *tree)
{
...
Bool_t distribMCForBins::Notify()
{
  return kTRUE;
}

void distribMCForBins::Show(Long64_t entry)
{
...
Int_t distribMCForBins::Cut(Long64_t entry)
{
...
#endif // #ifdef distribMCForBins_cxx
