...
void DoSPlot(RooWorkspace* wspace){
  ...
  // --------------------------------------
  // background model: K*Psi2s
  std::cout << "K*Psi2s background model" << std::endl;
  //
  RooRealVar *frac_partial = wspace->var("frac_partial");
  RooAbsPdf  *kstarpsi2s   = wspace->pdf("kstarpsi2s");
  ...
  SPlotPsi2s* sData = new SPlotPsi2s("sData","A SPlot", *mydatads, model, RooArgList(*nsignal, *notherB, *ncomb, *frac_partial) );
  ...
  std::cout << std::endl <<  "K*Psi2s background fraction is " << frac_partial->getVal() 
