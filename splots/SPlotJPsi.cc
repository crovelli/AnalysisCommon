...
SPlotJPsi::SPlotJPsi(const char* name, const char* title):
  TNamed(name, title)
{
  RooArgList Args;
...
SPlotJPsi::SPlotJPsi(const char* name, const char* title, const RooDataSet &data):
  TNamed(name, title)
{
  RooArgList Args;
...
////////////////////////////////////////////////////////////////////////////////
/// Copy Constructor from another SPlotJPsi

SPlotJPsi::SPlotJPsi(const SPlotJPsi &other):
  TNamed(other)
{
...
SPlotJPsi::SPlotJPsi(const char* name, const char* title, RooDataSet& data, RooAbsPdf* pdf,
  ...
  TNamed(name, title)
{
  ...
  while((arg=(RooAbsArg*)iter->Next())) {
    if (!dynamic_cast<RooRealVar*>(arg)) {
      coutE(InputArguments) << "SPlotJPsi::SPlotJPsi(" << GetName() << ") input argument "
             << arg->GetName() << " is not of type RooRealVar " << endl ;
      throw string(Form("SPlotJPsi::SPlotJPsi(%s) input argument %s is not of type RooRealVar",GetName(),arg->GetName())) ;
    }
  }
  delete iter ;

  //Construct a new SPlotJPsi class,
...
RooDataSet* SPlotJPsi::SetSData(RooDataSet* data)
{
  if(data)    {
    fSData = (RooDataSet*) data;
    return fSData;
...
RooDataSet* SPlotJPsi::GetSDataSet() const
{
  return fSData;
}
...
Double_t SPlotJPsi::GetSWeight(Int_t numEvent, const char* sVariable) const
{
...
Double_t SPlotJPsi::GetSumOfEventSWeight(Int_t numEvent) const
{
  if(numEvent > fSData->numEntries() )
    {
      coutE(InputArguments)  << "Invalid Entry Number" << endl;
...
Double_t SPlotJPsi::GetYieldFromSWeight(const char* sVariable) const
{
...
RooArgList SPlotJPsi::GetSWeightVars() const
{
...
Int_t SPlotJPsi::GetNumSWeightVars() const
{
  RooArgList Args = fSWeightVars;
...
///
/// The SPlotJPsi will contain two new variables for each specie of name "varname":
///
...
void SPlotJPsi::AddSWeight( RooAbsPdf* pdf, const RooArgList &yieldsTmp,
         const RooArgSet &projDeps, bool includeWeights)
{
  ...
  // This is necessary because SPlotJPsi assumes the yields minimise -Log(likelihood)
  ...
  for (Int_t ievt = 0; ievt <numevents; ievt++) {
	...
	cout << "SPlotJPsi varPippo: " << varPippo->GetName() << " " << varPippo->getVal() << endl;
	pdf->Print(); 
	cout << "SPlotJPsi pdf->getVal = " << pdf->getVal(&vars) << endl;
      }
      */
      //**************
  ...
  if (covInv.Determinant() <=0)
    {
      coutE(Eval) << "SPlotJPsi Error: covariance matrix is singular; I can't invert it!" << std::endl;
      covInv.Print();
      return;
    }
