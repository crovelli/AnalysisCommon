...
############################# total fit ##############################
def total_fit(tree, outputfile, branches, sgn_parameters=None, kjpsi_parameters=None, bkg_parameters=None, set_sgn_yield=None,Blind_range={"min":4.7,"max":5.7} , number_of_mctoys=None,mva=None,log='log.csv'):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ikjpsi", "--ikspiBkg", dest="ikspiBkg", default="../BDT/TrainingCore/forMeas_xgbmodel_kee_finalfix_12B_*_MCres.root", help="Input kjpsi BKG file")
    ...
    if args.fit_primtv:
      ...
      if "bkg_kjpsi" in args.sel_primtv:
        ...
        tree_kjpsi_cut=tree_kjpsi.CopyTree(cuts)
        kjpsi_parameters = kjpsi_fit(tree_kjpsi_cut, args.outputfile+"_kjpsiMC", branches)
        print "parameters KJpsi BKG", kjpsi_parameters['mean_kjpsi'], kjpsi_parameters['width_kjpsi']
    ...
    if not args.skip_realfit:
      ...
      tree_cut=tree.CopyTree(cuts)
      nsig, nbkg, nkjpsi =total_fit(tree_cut, args.outputfile, branches, signal_parameters,  kjpsi_parameters, bkg_parameters, args.set_expected_sgn, {"min":5.0,"max":5.4}, args.number_of_mctoys, str(args.mva), args.log)
